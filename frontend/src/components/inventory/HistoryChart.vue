...
function createChart() {
  ...
  // Adaptive point display based on data density
  const dataCount = sortedData.length
  const showPoints = dataCount < 30
  const linePointRadius = showPoints ? 4 : 0
  const linePointHoverRadius = showPoints ? 6 : 4
