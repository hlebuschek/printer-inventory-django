    ...
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title"><i class="bi bi-key me-2"></i>Токен M4</h5>
          <button type="button" class="btn-close" @click="close"></button>
        </div>
        <div class="modal-body">
          <p class="text-muted" style="font-size: 0.85rem;">
            Введите ваш токен M4 — заявки подрядчикам, работающим через M4, будут уходить от вашего имени.
          </p>
          <input type="password" class="form-control" v-model="token" placeholder="Токен"
                 @keyup.enter="save">
...
export default {
  ...
  mounted() {
    window.openM4Modal = () => this.open()
  },
  beforeUnmount() {
    delete window.openM4Modal
  },
  methods: {
    open() {
      this.token = ''
    ...
    async save() {
      ...
      try {
        ...
        const resp = await fetch('/api/m4-token/', {
          method: 'POST',
          headers: {
