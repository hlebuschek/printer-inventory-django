...
def run_inventory_for_printer(printer_id):
    ...
    async_to_sync(channel_layer.group_send)('inventory_updates', start_payload)
    ...
    # SNMP discovery
    disc_cmd = f'"{disc_exe}" -i --host {ip} --community {community} --save={OUTPUT_DIR} --debug'
    ok, out = run_glpi_command(disc_cmd)
    if not ok:
        InventoryTask.objects.create(printer=printer, status='FAILED', error_message=out)
        return False, out

    # SNMP inventory
    inv_cmd = f'"{inv_exe}" -i --host {ip} --community {community} --save={OUTPUT_DIR} --debug'
    ...
    if not data:
        InventoryTask.objects.create(printer=printer, status='FAILED', error_message='XML parse error')
