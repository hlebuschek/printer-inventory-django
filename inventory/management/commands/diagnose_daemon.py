...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        tasks_24h = InventoryTask.objects.filter(task_timestamp__gte=cutoff_24h)
        ...
        success_rate = (
            tasks_24h.filter(status="SUCCESS").count() / tasks_24h.count() * 100 if tasks_24h.count() > 0 else 0
        )
