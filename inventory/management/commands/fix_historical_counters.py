...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        # Фильтры устройств
        parser.add_argument("--model", type=str, help="Исправить для конкретной модели принтера")
        parser.add_argument("--printer-ip", type=str, help="Исправить для конкретного принтера по IP")
        parser.add_argument("--printer-serial", type=str, help="Исправить для конкретного принтера по серийному номеру")
        parser.add_argument("--all", action="store_true", help="Исправить для всех принтеров (осторожно!)")
        ...
        parser.add_argument(
            "--before", type=str, help="Исправить только задачи до указанной даты (YYYY-MM-DD или YYYY-MM-DD HH:MM:SS)"
        )

        # Дополнительные опции
        parser.add_argument("--dry-run", action="store_true", help="Показать что будет изменено, без сохранения")
        parser.add_argument("--limit", type=int, default=None, help="Ограничить количество обрабатываемых задач")
        parser.add_argument("--force", action="store_true", help="Принудительно исправить даже уже цветные записи")
    ...
    def handle(self, *args, **options):
        ...
        if not any([options["model"], options["printer_ip"], options["printer_serial"], options["all"]]):
            raise CommandError("Укажите один из фильтров: --model, --printer-ip, --printer-serial или --all")
        ...
        tasks_qs = (
            InventoryTask.objects.filter(printer__in=printers, status="SUCCESS")
            .select_related("printer")
            .prefetch_related("pagecounter_set")
        )
        ...
        with transaction.atomic():
            for task in tasks:
                try:
                    ...
                    error_count += 1
                    self.stdout.write(self.style.ERROR(f"Ошибка при обработке задачи {task.id}: {e}"))
    ...
    def _process_task(self, task, force=False):
        ...
        changes_needed = (
            ...
            or page_counter.bw_a4 != new_counters["bw_a4"]
            or page_counter.color_a3 != new_counters["color_a3"]
            or page_counter.color_a4 != new_counters["color_a4"]
        )
