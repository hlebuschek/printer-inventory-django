...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        old_count = InventoryTask.objects.filter(task_timestamp__lt=archive_boundary).count()
        ...
        if old_count > 0:
            ...
            if not dry_run:
                with transaction.atomic():
                    deleted = InventoryTask.objects.filter(task_timestamp__lt=archive_boundary).delete()
        ...
        for printer in printers:
            deleted = self._cleanup_printer_archive(printer, keep_boundary, archive_boundary, dry_run)
        ...
        recent_tasks = InventoryTask.objects.filter(task_timestamp__gte=keep_boundary).count()

        archive_tasks = InventoryTask.objects.filter(
            task_timestamp__lt=keep_boundary, task_timestamp__gte=archive_boundary
        ).count()
