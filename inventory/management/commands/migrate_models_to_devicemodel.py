...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.MIGRATE_HEADING(
            '🔄 Миграция моделей принтеров в DeviceModel'
        ))
        ...
        for printer in printers_qs.select_related('device_model'):
            ...
            if not device_model:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"⚠️  Модель не найдена в справочнике: '{model_text}' "
        ...
        if not_found:
            self.stdout.write(
                self.style.ERROR(f"  Не найдено в справочнике: {not_found}")
            )
            self.stdout.write(
                self.style.WARNING(
                    '\n💡 Совет: Добавьте отсутствующие модели в справочник contracts '
