...
def api_printers(request):
    ...
    if printer_ids:
        for printer_id in printer_ids:
            task = (
                InventoryTask.objects.filter(printer_id=printer_id, status="SUCCESS")
                ...
                "device_model": (
                    {
                        ...
                        "manufacturer": (
                            {"id": p.device_model.manufacturer_id, "name": p.device_model.manufacturer.name}
                            if p.device_model.manufacturer
                            else None
                        ),
                    }
                    if p.device_model
    ...
    manufacturers = list(
        Manufacturer.objects.filter(models__device_type="printer").distinct().order_by("name").values("id", "name")
    )
    ...
    device_models = list(
        DeviceModel.objects.filter(device_type="printer").order_by("name").values("id", "name", "manufacturer_id")
    )

    organizations = list(Organization.objects.filter(active=True).order_by("name").values("id", "name"))
...
def api_printer(request, pk):
    ...
    printer = get_object_or_404(
        Printer.objects.select_related("organization", "device_model", "device_model__manufacturer"), pk=pk
    )

    # Получаем последнюю успешную задачу
    task = (
        InventoryTask.objects.filter(printer_id=pk, status="SUCCESS")  # Используем printer_id для консистентности
        .order_by("-task_timestamp")
        .first()
    )
...
def api_models_by_manufacturer(request):
    ...
    try:
        models = (
            DeviceModel.objects.filter(manufacturer_id=manufacturer_id, device_type="printer")
        ...
        return JsonResponse(
            {
                "models": [
                    {"id": m["id"], "name": m["name"], "full_name": f"{m['manufacturer__name']} {m['name']}"}
                    for m in models
                ]
            }
        )
...
def api_all_printer_models(request):
    ...
    models = (
        DeviceModel.objects.filter(device_type="printer")
        .select_related("manufacturer")
        .order_by("manufacturer__name", "name")
        .values("id", "name", "manufacturer__name", "manufacturer__id")
    )
...
def api_system_status(request):
    ...
    recent_success = (
        InventoryTask.objects.filter(task_timestamp__gte=recent_cutoff, status="SUCCESS")
        .values("printer")
        .distinct()
        .count()
    )
...
def api_status_statistics(request):
    ...
    status_stats = (
        InventoryTask.objects.filter(task_timestamp__gte=start_date)
        .values("status")
    ...
    problematic_printers = (
        InventoryTask.objects.filter(
            task_timestamp__gte=start_date, status__in=["FAILED", "VALIDATION_ERROR", "HISTORICAL_INCONSISTENCY"]
        )
        .values("printer__ip_address", "printer__device_model__name", "status")
...
def api_printer_replacement_history(request, pk):
    ...
    changes = (
        PrinterChangeLog.objects.filter(Q(printer=printer) | Q(related_printer=printer))
        .select_related("printer", "related_printer")
