...
def autolink_printer(sender, instance: ContractDevice, **kwargs):
    ...
    try:
        ...
        matching_printers = Printer.objects.filter(serial_number__iexact=sn, is_active=True)
        ...
        logger.info(
            f"Устройство с серийником {sn} автоматически связано с принтером " f"ID:{printer.id}({printer.ip_address})"
        )
...
def link_printer_on_save(sender, instance, created, **kwargs):
    ...
    try:
        ...
        unlinked_devices = ContractDevice.objects.filter(serial_number__iexact=sn, printer__isnull=True)
        ...
        if linked_count > 0:
            logger.info(f"Новый принтер ID:{instance.id} связан с {linked_count} устройствами по серийнику {sn}")
