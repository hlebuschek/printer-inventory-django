...
class BulkChangeStatusForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if exclude_statuses:
            self.fields["new_status"].queryset = self.fields["new_status"].queryset.exclude(id__in=exclude_statuses)
...
class BulkChangeServiceMonthForm(forms.Form):
    ...
    new_service_month = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(attrs={"type": "month", "class": "form-control", "placeholder": "YYYY-MM"}),
    ...
    def clean(self):
        ...
        if clear_service_month and new_service_month:
            raise forms.ValidationError(
                "Нельзя одновременно установить новый месяц и очистить его. " "Выберите что-то одно."
            )

        if not clear_service_month and not new_service_month:
            raise forms.ValidationError(
                "Необходимо либо выбрать новый месяц обслуживания, " "либо поставить галочку для очистки."
            )
...
class BulkChangeStatusAndServiceMonthForm(forms.Form):
    ...
    def clean(self):
        ...
        if clear_service_month and new_service_month:
            raise forms.ValidationError("Нельзя одновременно установить новый месяц и очистить его.")
