...
4. Валидация данных (inventory/utils.py:validate_against_history)
   ...
   - 🛡️ ЗАЩИТА ОТ АНОМАЛЬНЫХ СКАЧКОВ (Kyocera bug):
     • Если последний опрос был недавно (< 2 часов)
     • И счетчик резко вырос (+100,000 страниц или > 1.5x)
...
ANOMALY_CHECK_TIME_WINDOW_HOURS=2  # По умолчанию: 2 часа

# Абсолютное увеличение счетчика, считающееся аномальным (страниц)
ANOMALY_HUGE_JUMP_THRESHOLD=100000  # По умолчанию: 100,000 страниц

# Коэффициент увеличения, считающийся подозрительным (например, 1.5 = увеличение в 1.5 раза)
ANOMALY_SUSPICIOUS_RATIO=1.5  # По умолчанию: 1.5

# Минимальное абсолютное увеличение для проверки коэффициента (страниц)
ANOMALY_RATIO_MIN_INCREASE=50000  # По умолчанию: 50,000 страниц
```
