...
class DjangoAuthMixin:
    ...
    def login(self):
        ...
        response = self.client.get("/accounts/django-login/", name="/accounts/django-login/ [GET]")
        ...
        if "csrftoken" not in self.client.cookies:
            ...
            self.client.cookies.set(name="csrftoken", value=csrf_token, domain=domain, path="/")
...
class KeycloakAuthMixin:
    ...
    def login(self):
        ...
        try:
            ...
            callback_url = self._submit_keycloak_login(keycloak_login_url, keycloak_data)
    ...
    def _initiate_oidc_auth(self):
        ...
        """
        response = self.client.get("/oidc/authenticate/", name="/oidc/authenticate/ [GET]", allow_redirects=False)
    ...
    def _get_keycloak_login_form(self, auth_url):
        ...
        with self.client.get(
            auth_url, name="[Keycloak] GET login form", catch_response=True, allow_redirects=True
        ) as response:
            ...
            for match in re.finditer(r'<input[^>]*type="hidden"[^>]*name="([^"]+)"[^>]*value="([^"]*)"', response.text):
    ...
    def _submit_keycloak_login(self, login_url, form_data):
        ...
        with self.client.post(
            login_url, data=form_data, name="[Keycloak] POST login", catch_response=True, allow_redirects=False
        ) as response:
            response.success()  # Всегда считаем успешным
    ...
    def _process_oidc_callback(self, callback_url):
        ...
        response = self.client.get(callback_path, name="/oidc/callback/ [GET]", allow_redirects=True)
        ...
        check_response = self.client.get("/inventory/", name="[check auth] /inventory/", catch_response=True)
