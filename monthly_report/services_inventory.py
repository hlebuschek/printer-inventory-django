...
def get_inventory_suggestions(pairs: Iterable[Tuple[str | None, str | None]]) -> Dict[Tuple[str, str], Dict[str, Any]]:
    """
    ...
    printers = Printer.objects.filter(serial_number__in=list(serial_numbers)).values(
        "id", "serial_number", "ip_address", "last_updated"
    )

    # Создаем маппинг serial_number -> printer_data
    printer_map = {
        p["serial_number"]: {"id": p["id"], "ip": p["ip_address"], "last_updated": p["last_updated"]} for p in printers
    }
...
def get_inventory_latest_counters(serial_numbers: Iterable[str]) -> Dict[str, Dict[str, Any]]:
    """
    ...
    data = (
        PageCounter.objects.filter(task__printer__serial_number__in=serial_list, task__status="SUCCESS")
