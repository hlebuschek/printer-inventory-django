...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("month", type=str, help="Месяц в формате YYYY-MM (например, 2025-11)")
    ...
    def handle(self, *args, **options):
        ...
        if count == 0:
            self.stdout.write(self.style.WARNING(f"Нет данных за {month_str}"))
            return
        ...
        sample_before = list(
            MonthlyReport.objects.filter(month=month_date)[:5].values(
        ...
        sample_after = list(
            MonthlyReport.objects.filter(id__in=[r["id"] for r in sample_before]).values(
                "id", "serial_number", "a4_bw_start", "a4_bw_end", "total_prints"
            )
        )
        ...
        negative_count = MonthlyReport.objects.filter(month=month_date, total_prints__lt=0).count()

        if negative_count > 0:
            self.stdout.write(self.style.WARNING(f"\n⚠ Найдено {negative_count} записей с отрицательными значениями"))
            # Показываем примеры отрицательных
            negative_samples = MonthlyReport.objects.filter(month=month_date, total_prints__lt=0)[:10].values(
                "id",
                "serial_number",
                "equipment_model",
                "a4_bw_start",
