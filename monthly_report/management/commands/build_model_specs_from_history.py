...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--apply", action="store_true", help="По умолчанию DRY-RUN. С --apply изменения сохраняются."
        )
        parser.add_argument(
            "--only-missing", action="store_true", help="Обновлять только отсутствующие записи справочника."
        )
        parser.add_argument(
            "--since",
            type=str,
    ...
    def handle(self, *args, **opt):
        ...
        for raw_name in model_names:
            ...
            qs = (
                qs.filter(equipment_model__iexact=name)
                if not opt["case_sensitive"]
                else qs.filter(equipment_model=name)
            )
            ...
            agg = qs.annotate(
                ...
                any_color=Sum(
                    Case(When(Q(a4_c__gt=0) | Q(a3_c__gt=0), then=1), default=0, output_field=IntegerField())
                ),
            ...
            to_update = []
            if not created:
                if spec.is_color != any_color:
                    spec.is_color = any_color
                    to_update.append("is_color")
                if spec.paper_format != fmt:
                    spec.paper_format = fmt
                    to_update.append("paper_format")
                if spec.enforce != enforce:
                    spec.enforce = enforce
                    to_update.append("enforce")
        ...
        if opt["rewrite_existing"]:
            if dry:
                self.stdout.write(
                    self.style.WARNING("DRY-RUN: пропускаю перепись существующих строк (--apply не задан).")
                )
            else:
    ...
    def _rewrite_existing(self, month_filter: dict, case_sensitive: bool):
        ...
        from monthly_report.specs import allowed_counter_fields, get_spec_for_model_name
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Переписано строк: {changed_rows}, месяцев пересчитано: {len(months_touched)}")
        )
