...
## ⚠️ ВАЖНО: Проверка версии Node.js

**КРИТИЧНО:** На машине с интернетом и на сервере должна быть **ОДИНАКОВАЯ** мажорная версия Node.js!

### Почему это важно?

Vite использует **native бинарники** (esbuild, rollup), которые зависят от версии Node.js:
- ❌ **Сборка на Node 24 → Установка на Node 18** = Могут быть ошибки!
- ✅ **Сборка на Node 18 → Установка на Node 18** = Работает отлично!

### Проверка версий:

```bash
# На MacBook:
node -v  # Например: v24.3.0 ❌

# На сервере:
ssh server "node -v"  # v18.20.4 ✅

# ВЕРСИИ РАЗНЫЕ! Нужно исправить
```

### Решение 1: Использовать nvm на MacBook (РЕКОМЕНДУЕТСЯ)

```bash
# 1. Установите nvm (если ещё нет)
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.7/install.sh | bash
source ~/.zshrc  # или ~/.bash_profile

# 2. Установите Node.js 18 (как на сервере)
nvm install 18
nvm use 18
...
# 4. Теперь можно создавать архив!
```

### Решение 2: Использовать готовый скрипт

Проект включает скрипт для автоматической сборки с правильной версией:
...
# Скрипт автоматически:
# - Переключится на Node.js 18
# - Установит зависимости
# - Создаст архив npm-dependencies.tar.gz
```

### Автоматическое переключение версии

Проект содержит файл `.nvmrc` с версией Node.js 18:

```bash
# Просто выполните в директории проекта:
nvm use

# nvm автоматически переключится на Node.js 18
```

### Подробная документация

См. [NODE_VERSION_COMPATIBILITY.md](NODE_VERSION_COMPATIBILITY.md) для:
- Использования Docker для сборки
