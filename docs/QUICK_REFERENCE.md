...
### 8. Кастомные права доступа (v2.1.0+)
**10 кастомных прав для monthly_report:**
- `access_monthly_report` - доступ к модулю
- `upload_monthly_report` - загрузка Excel
- `edit_counters_start` / `edit_counters_end` - редактирование счетчиков
- `sync_from_inventory` - синхронизация
- `view_change_history` - история изменений
- `can_manage_month_visibility` - публикация/скрытие отчетов
- `can_reset_auto_polling` - возврат на автоопрос
- `can_poll_all_printers` - массовый опрос

**11 групп прав** для удобного управления (см. `python manage.py init_monthly_report_roles`)
...
1. Администратор импортирует Excel с данными оборудования
   - Можно указать статус публикации (is_published)
   - По умолчанию отчет создается скрытым для проверки
...
6. Проверка и публикация отчета:
...
7. Возврат на автоопрос (при необходимости):
   - Если пользователь случайно отредактировал счетчик вручную
   - В истории изменений есть кнопка "Вернуть на автоопрос"
   - Сбрасываются все флаги manual_edit_* для устройства
8. Экспорт обратно в Excel для выставления счетов/соответствия
```
