...
- ✅ **Управление видимостью месяцев** (публикация/скрытие отчётов)
- ✅ **Возврат на автоопрос** (сброс ручных изменений)
- ✅ **Массовый опрос принтеров** (запуск опроса для всех устройств)
...
- **Управление видимостью (v2.1.0+):**
  - Кнопка "Опубликовать"/"Скрыть" (требуется право `can_manage_month_visibility`)
  - Неопубликованные отчеты видят только пользователи с правом управления
  - Индикатор статуса публикации на карточке месяца
...
#### 3.1. Кастомные права доступа

Система включает **10 кастомных прав** для детального управления доступом к функциям:
...
**Продвинутые функции (v2.1.0+):**
- `can_manage_month_visibility` - Управление видимостью месяцев (публикация/скрытие)
- `can_reset_auto_polling` - Возврат принтера на автоопрос (сброс флагов `manual_edit_*`)
- `can_poll_all_printers` - Опрос всех принтеров одновременно (массовый опрос)

**Группы прав:**
Для удобного управления доступом созданы 11 групп (см. `python manage.py init_monthly_report_roles`):
- Просмотр
- Загрузка
- Редакторы (начало/конец/полные)
- Синхронизация
- Менеджеры (все права)
- История
...
#### 3.2. Система разрешений на уровне полей

Трёхуровневая проверка разрешений для редактирования ячеек:
...
const resetting = ref(false)        // Сброс флагов в процессе
const permissions = ref({})         // Права пользователя
...
- `POST /monthly-report/api/reset-manual-flag/<report_id>/` - возврат на автоопрос (v2.1.0+)
...
resetManualFlags()                 // Вернуть на автоопрос (v2.1.0+)
...
- **Возврат на автоопрос (v2.1.0+):**
  - Кнопка "Вернуть на автоопрос" (требуется право `can_reset_auto_polling`)
  - Сбрасывает все флаги `manual_edit_*` для устройства
  - Восстанавливает автоматические значения из inventory
...
### Переключение публикации месяца (v2.1.0+)

```http
POST /monthly-report/api/toggle-month-published/<year>/<month>/
X-CSRFToken: <token>
```

**Требуется право:** `can_manage_month_visibility`
...
```json
{
  "ok": true,
  "is_published": true,
  "message": "Отчёт за 2025-11 опубликован"
}
```
...
```http
POST /monthly-report/api/reset-manual-flag/<report_id>/
X-CSRFToken: <token>
```

**Требуется право:** `can_reset_auto_polling`

**Описание:** Сбрасывает все флаги `manual_edit_*` для указанного устройства и восстанавливает автоматические значения из inventory.

**Response:**
```json
{
  "ok": true,
  "message": "Флаги ручного редактирования сброшены. Устройство вернулось на автоопрос.",
  "fields_reset": ["a4_bw_end", "a4_color_end"]
}
```
