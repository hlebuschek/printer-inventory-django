...
# Page counter OIDs for A3 and A4
use constant canonBlackA3Print  => countersC55XX . '.112'; # .1.3.6.1.4.1.1602.1.11.1.3.1.4.112
use constant canonBlackA4Print  => countersC55XX . '.113'; # .1.3.6.1.4.1.1602.1.11.1.3.1.4.113
...
sub run {
    ...
    # Add A3 and A4 black and white counters
    my $black_a3 = $self->get(canonBlackA3Print);
    my $black_a4 = $self->get(canonBlackA4Print);

    $device->{PAGECOUNTERS}->{BW_A3} = $black_a3 if defined $black_a3;
    $device->{PAGECOUNTERS}->{BW_A4} = $black_a4 if defined $black_a4;
...
This module enhances Canon printers support, including page counters for A3 and A4 formats for black and white printing.
