...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("  Очередь: high_priority")
        if update_only:
            self.stdout.write("  Режим: Только обновление договора (без проверки)")
        elif update_contract:
            self.stdout.write("  Режим: Проверка + обновление поля договора")
        else:
            self.stdout.write("  Режим: Только проверка наличия")
        self.stdout.write("")
        ...
        if options["sync"]:
            ...
            self.stdout.write("   • Или запустить с --sync для отслеживания")
            self.stdout.write("   • Логи задачи: tail -f logs/celery.log | grep -i glpi")
            self.stdout.write("")
