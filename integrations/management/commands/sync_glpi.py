...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--force", action="store_true", help="Принудительная проверка (игнорировать кэш 1 час)")
        parser.add_argument(
            "--with-serial", action="store_true", help="Проверять только устройства с серийными номерами"
        )
        parser.add_argument("--device-ids", nargs="+", type=int, help="ID конкретных устройств для проверки")
        parser.add_argument(
            "--delay", type=float, default=0.5, help="Задержка между запросами в секундах (по умолчанию 0.5)"
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        if options["device_ids"]:
            ...
            if options["with_serial"]:
                ...
                self.stdout.write("Фильтр: только устройства с серийниками")
        ...
        for index, device in enumerate(devices, 1):
            ...
            if not device.serial_number:
                stats["skipped"] += 1
                self.stdout.write(f"[{index}/{total}] ПРОПУЩЕН: {device} (нет серийника)")
                continue

            # Проверяем устройство
            try:
                sync = check_device_in_glpi(device, user=None, force_check=options["force"])
                ...
                self.stdout.write(f"[{index}/{total}] {device.serial_number:20} | {status_style(status_msg)}")

            except Exception as e:
                stats["errors"] += 1
                self.stdout.write(
                    f"[{index}/{total}] {device.serial_number:20} | " f"{self.style.ERROR(f'✗ Исключение: {e}')}"
                )
