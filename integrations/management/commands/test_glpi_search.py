...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--verbose", action="store_true", help="Показывать детальную информацию")
        ...
        try:
            ...
            with GLPIClient() as client:
                ...
                if client.session_token:
                    token_preview = (
                        client.session_token[:20] + "..." if len(client.session_token) > 20 else client.session_token
                    )
                ...
                if status == "ERROR":
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"⚠ Принтер с серийным номером '{serial_number}' НЕ найден в GLPI")
                    )
                    self.stdout.write("")
