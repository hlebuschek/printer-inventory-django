...
class EntityChangeLog(models.Model):
    ...
    changes = models.JSONField(
        default=dict, blank=True, verbose_name="Изменения", help_text="Словарь {поле: {old: старое, new: новое}}"
    )
...
class AllowedUser(models.Model):
    ...
    email = models.EmailField(
        blank=True, verbose_name="Email (опционально)", help_text="Для справки, не используется при проверке"
    )
    full_name = models.CharField(max_length=255, blank=True, verbose_name="ФИО (опционально)")
    is_active = models.BooleanField(
        default=True, verbose_name="Активен", help_text="Можно временно отключить доступ без удаления"
    )
...
class UserThemePreference(models.Model):
    ...
    user = models.OneToOneField(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="theme_preference", verbose_name="Пользователь"
    )
