...
class AllowedUserAdmin(admin.ModelAdmin):
    list_display = ("username", "email", "full_name", "colored_status", "has_django_user", "added_at", "added_by")
    ...
    fieldsets = (
        ("Основная информация", {"fields": ("username", "is_active")}),
        ("Дополнительная информация", {"fields": ("email", "full_name", "notes")}),
        ("Системная информация", {"fields": ("added_at", "added_by"), "classes": ("collapse",)}),
    )
    ...
    def activate_users(self, request, queryset):
        ...
        for allowed_user in queryset:
            django_user = User.objects.filter(username__iexact=allowed_user.username).first()
            if django_user and not django_user.is_superuser:
...
class EntityChangeLogAdmin(admin.ModelAdmin):
    list_display = ("id", "content_type", "object_repr", "action_display", "user_display", "timestamp", "ip_address")
    ...
    fieldsets = (
        ("Объект", {"fields": ("content_type", "object_id", "object_repr")}),
        ("Действие", {"fields": ("action", "user", "timestamp")}),
        ("Изменения", {"fields": ("changes",)}),
        ("Метаданные", {"fields": ("ip_address", "user_agent"), "classes": ("collapse",)}),
    )

    def action_display(self, obj):
        colors = {"create": "green", "update": "blue", "delete": "red"}
